...
public class Controller extends HttpServlet {
	...
	protected void proc(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(cmd.equals("main")){
			...
			int pagesize = 5;
			viewpage = "/main.jsp";
			obj = dao.selectAll(getConnection(),curpage,pagesize);
			...
			int result = dao.delete(getConnection(),request.getParameter("idx"));
			if(result>0){
			...
			int result = dao.idChk(getConnection(),id);
			if(result>0){
				viewpage="/signupform.jsp";
				dto.setId("사용할 수 없는 아이디");
				request.setAttribute("result", dto);
			}else{
				result = dao.insert(getConnection(),dto);
				if(result>0){
					response.sendRedirect(request.getContextPath());
				}else{
					viewpage = "/complete.jsp";
			...
			MDto dto = new MDto(id, pw, null);
				int result = dao.signin(getConnection(),dto);
				if(result>0){
			...
			MDto dto = dao.selectOne(getConnection(),(String)request.getSession(false).getAttribute("authid"));
			request.setAttribute("result", dto);
			...
			int result = dao.moduser(getConnection(),dto, oldpw);
			if(result>0){
