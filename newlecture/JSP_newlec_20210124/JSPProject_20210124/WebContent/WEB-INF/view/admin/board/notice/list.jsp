...
<body>
	...
	<div id="body">
		...
		<c:set var="lastNum" value="${fn:substringBefore(Math.ceil(count/10),'.')}"></c:set>
					...
					<tr>		
						...
						<td>${n.writer_id}</td>
						<td>
						<fmt:formatDate value="${n.regdate}" pattern="yyyy년 M월 d일"/>
								
						</td>
						<td><fmt:formatNumber value="${n.hit}" /></td>
						<td><input type="checkbox" name="open-id" ${n.pub?"checked":""} value="${n.id}"></td>
	...
	<ul class="-list- center">
		...
		<li><a class="-text- ${page==startNum+i? 'orange':''} bold" href="?p=${i+startNum}&t=&q=" >${startNum+i}</a></li>
		</c:if>
		</c:forEach>
	</ul>
	<div>
